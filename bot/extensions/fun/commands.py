...
from asyncio import Lock, sleep, gather, create_task, as_completed, ensure_future, CancelledError, all_tasks
...
class Fun(Cog):
    ...
    @command(name = "pack", description = "blaze that nigga yahurd", example = ",pack jonathan")
    async def pack(self, ctx: Context, *, member: Member):
        async with self.locks["pack"]:
            tasks = [create_task(ctx.send(f"{member.mention} {next(self.pack_bible)}")) for i in range(5)]
    ...
    async def do_fetch(self, channel: TextChannel, limit: int = 5000) -> List[Message]:        
        messages = [m async for m in channel.history(limit=limit) if m.content is not None]
    ...
    async def get_guild_messages(self, guild: Guild):
        messages = list(chain.from_iterable(await gather(*[self.bot.redis.get_all_messages(guild, u) for u in guild.members])))
...
#    @offloaded
    def _generate_wc(self, data: dict):
        ...
        kwargs = {
            ...
            "color_func": None,
            "mode": "RGB",
            "font_path": "data/bebasneue-wordcloud.ttf",
    ...
    async def get_global_messages(self, user: Union[Member, User]):
        if user := self.bot.get_user(user):
            guilds = {str(g.id):[str(c.id) for c in g.text_channels] for g in user.mutual_guilds}
            id = user.id
        else:
            guilds = {}
            id = user
        for source in self.bot.sources:
            if source != self.bot.cluster_name:
                data = await self.bot.ipc.request('get_wc_guilds',source = source, user_id = id)
                guilds.update(data)
        messages = list(chain.from_iterable(await gather(*[self.bot.redis.get_all_messages(guild, user, channels) for guild, channels in guilds.items()])))
        messages = [i.decode() for i in messages]
#        messages = [m.encode('UTF-8').decode('UTF-8') for m in messages]
    ...
    async def do_wc(
        self, ctx, user: Optional[Member] = None, channel: Optional[TextChannel] = None, is_global: Optional[bool] = False
    ):
    ...
    async def wordcloud(
        self, ctx, user: Optional[Union[User, Member]] = None, channel: Optional[TextChannel] = None
    ):
    ...
    @wordcloud.command(name = "global", description = "make a wordcloud of a user's global messages", with_app_command = True)
    async def wordcloud_global(self, ctx, *, user: Optional[Union[Member, User]] = None):
        user = user or ctx.author
        message = await ctx.normal("please wait while I fetch all of the messages needed..")
        async with timeit() as timer:
            try:
                text = await self.get_global_messages(user.id or user)
                file = await generate_wc({'text': text})
            except Exception:
                return await message.edit(embed = Embed(color = self.bot.color, description = "no **messages** found for that user"))
        return await message.edit(attachments = [File(fp = BytesIO(file), filename = "wordcloud.png")], embed = Embed(color = self.bot.color, description = f"{self.bot.config['emojis']['success']} {ctx.author.mention}: successfully generated the wordcloud").set_footer(text = f"took {int(timer.elapsed)} seconds"))
    ...
    @hybrid_group(name = "blacktea", description = "Find a word with 3 letters!", invoke_without_command = True)
    @blacktea_round()
    async def blacktea(self: "Fun", ctx: Context, *, flags: BlackTeaFlags):
        return await create_task(start_blacktea(ctx, life_count=flags.lives, timeout=flags.timeout), name = f"blacktea-{ctx.guild.id}")
    
    @blacktea.command(name = "end", aliases = ["stop"], description = "end an existing game of blacktea")
