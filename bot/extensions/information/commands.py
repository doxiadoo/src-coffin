...
from discord.utils import chunk_list, escape_markdown as escape_md, format_dt, utcnow, oauth_url
from system.classes.builtins import human_timedelta
from discord.abc import GuildChannel
from datetime import datetime
from asyncio import create_subprocess_shell
from system.classes.exceptions import NSFWDetection, ConcurrencyLimit
...
class AuditLogParser:
	...
	def do_parse(self):
		...
		i = 0
		previous_author_id = None  # Track the previous author ID to check for duplicates
		...
		for log in self.logs:
			...
			field_name = "\u200b" if log.user.id == previous_author_id else f"{log.user} ({log.user.id})"
...
class Information(Cog):
	...
	@hybrid_command(name = "status", description = "get information on discord's current status")
	...
	@command(name = "screenshot", description = "screenshot a webpage", example = ",screenshot https://google.com --wait 3", aliases = ["ss"])
	async def screenshot(self, ctx: Context, url: str, *, flags: ScreenshotFlags):
		...
		async with ClientSession() as session:
			async with session.request("HEAD", f"https://{url.replace('https://', '').replace('http://', '')}") as response:
				if int(response.headers.get("Content-Length", 5)) > 52428800 or url.endswith(".txt"):
					raise CommandError("Content Length Too Large")
		try:
			ss = await screenshot(url, safe, **kwargs)
			embed = Embed(title = f"{url.split('://')[1] if '://' in url else url}", url = f"https://{url.split('://')[1] if '://' in url else url}").set_image(url = "attachment://screenshot.png")
	...
	async def patch_help(self, ctx: Context):
		...
		for i, cog in enumerate(cogs, start = 1):
			...
			embed = Embed(
				title = cog.qualified_name.replace("_", " ").replace("Events", "").replace("Commands", "").title(),
				color=ctx.bot.color,
				url="https://coffin.bot/commands",
	...
	@hybrid_command(name = "help", aliases = ["cmds", "commands", "h"], description = "get information regarding a group or command", example = ",help antinuke")
	async def help_(self, ctx: Context, *, command_or_group: Optional[CommandorGroup] = None):
		try:
			h = Help()
			h.context = ctx
			if "--simple" in ctx.message.content.lower() or "-simple" in ctx.message.content.lower():
	...
	@group(name = "color", aliases = ["dominant", "pfpcolor", "avcolor"], description = "get information about a color, image color, or avatar color", example = ",color @aiohttp", invoke_without_command = True)
	...
	@color.command(name = "search", description = "search for a color by name", example = ",color purple")
	...
	@command(name = "donate", description = "support coffin thru various payment methods")
	async def donate(
		self, 
		ctx: Context
	) -> Message:
		await ctx.send(embed=Embed(color=self.bot.color, title="coffin â€” donate", description=f">>> LTC: `LLcWDBf7SkiES7m5F1Y8WR9ZftBEzA7XAs`\nBTC: `bc1qgydcnwzvar6hp3z4nng4kge06x8jxfrm965m3f`\nCashApp: [`$nycsluts`](<https://cash.app/$nycsluts>)\nOr boost our [**`support server`**]({CONFIG['SUPPORT_URL']})"))

	@command(name = "perks", description = "get information on coffin's perks")
	async def perks(
		self, 
		ctx: Context
	) -> Message:
		await ctx.send(embed=Embed(color=self.bot.color, title="coffin â€” perks", description=f">>> makemp3\nreskin\nvideotogif\nmore coming soon, give ideas in our [support server]({CONFIG['SUPPORT_URL']})"))
	...
	@command(name = "support", aliases=["discord"], description = "Get an invite link for the bot's support server.")
	async def support(self, ctx: Context) -> Message:
		return await ctx.reply(CONFIG["support_url"])
		
	@hybrid_command(name = "credits", aliases=["creds", "devs", "developers" "dev"], description = "Get credits on the bot's contributor's team")
	@app_commands.allowed_installs(guilds=True, users=True)
	@app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
	async def credits_(self, ctx: Context):
		embed = (
		Embed().set_footer(text=f"{self.bot.user.name} credits â€¢ {self.bot.user.name}.lol").set_thumbnail(url=self.bot.user.avatar))
		embed.add_field(
			name="Credits",
			value=f"`1` **{await self.bot.fetch_user(99965114706296832)}** - Founder & Developer (`99965114706296832`)\n`2` **{await self.bot.fetch_user(1188955485462872226)}** - Owner & Developer (`1188955485462872226`)\n`3` **{await self.bot.fetch_user(1280856653230505994)}** - Owner & Developer (`1280856653230505994`)"
		)
		view = View().add_item(
	...
	@hybrid_command(name = "botinfo", aliases=["bi", "bot", "info", "about"], description = "get information on the bot")
	@app_commands.allowed_installs(guilds=True, users=True)
	@app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
	async def botinfo(self, ctx: Context):
		stat = self.bot.statistics
		embed = Embed(
		...
		total_space = usage.total / (1024 ** 3)  # Convert bytes to GB  # noqa: F841
...
**Files**: {stat.files}""",
		...
		embed.description = f"{self.bot.config['emojis']['uptime']} {utils.format_dt(self.bot.startup_time, style='R')}"
		embed.set_author(name=self.bot.user.name, icon_url=self.bot.user.avatar)
		embed.set_thumbnail(url=self.bot.user.avatar)
		await ctx.send(embed=embed)
	
	@command(name = "audit", aliases = ["auditlogs"], description = "view the most recent audit logs for the guild")
	...
	async def audit(self, ctx: Context):
		...
		parser = AuditLogParser(self.bot, [entry async for entry in ctx.guild.audit_logs()])
		...
		def style(embed: Embed, footer: str):
			...
			return embed
		embeds = [style(e, f"Page {i}/{len(embeds)} ({log_count} {'entry' if log_count < 1 else 'entries'})") for i, e in enumerate(embeds, start = 1)]
	...
	async def bans(self, ctx: Context):
		bans = [f"{entry.user} ({entry.user.id}) - {entry.reason or 'No reason provided'}" async for entry in ctx.guild.bans()]
	...
	async def boosters(self, ctx: Context):
		...
		if not (rows := [f"{member.mention} {format_dt(member.premium_since, style='R')}" for member in boosters]):
			raise CommandError("There are no boosters in this server")
		
		return await ctx.paginate(
			Embed(title="Boosters"),
			rows,
			True
		)
	...
	async def bots(self, ctx: Context):
		if not (
			bots := sorted(
				...
				key = lambda x: x.joined_at, reverse = True
			)
		):
	...
	async def members(self, ctx: Context):
		...
		return await ctx.paginate(Embed(title = f"Members in {ctx.guild.name.shorten(25)}"), [f"{member.mention} - {format_dt(member.joined_at, style='R')}" for member in members], True)
	...
	@command(name = "emojis", aliases = ["emotes"], description = "view the emojis in the current guild")
	...
	@command(name = "avatar", description = "View a user's avatar", example = ",avatar @aiohttp", aliases = ["av"])
	async def avatar(self, ctx: Context, *, user: Optional[Union[Member, User]] = None):
		...
		return await ctx.send(
			embed=Embed(title=f"{user.name}'s avatar", description=f"[Click here to download]({user.display_avatar})",url=user.display_avatar)
			#   .set_author(name=f"{user.name}'s avatar")
			.set_image(url=user.display_avatar.url),
			#   view=view
		)
	
	@command(name = "banner", description = "View a user's banner", example = ",banner @aiohttp")
	...
	async def uptime(self: "Information", ctx: Context) -> Message:
		return await ctx.reply(
			embed=Embed(
				description=f"â° **{self.bot.user.display_name}** has been up for: {precisedelta(datetime.now() - self.bot.startup_time, format='%0.0f')}"
			)
		)

	@command(name="itemshop", aliases=["fnshop"], description = "View the Fortnite item shop")
	async def fortnite_shop(self: "Information", ctx: Context) -> Message:
		...
		embed = Embed(title = "Current Fortnite Shop")  # noqa: F841
		return await ctx.send(file=File(BytesIO(buffer), filename="shop.png"))

	@hybrid_command(name = "membercount", aliases=["mc"], description = "Get the amount of members in this server")
	...
	@command(name = "image", description = "Search Google for an image", example = ",image purple")
	async def image(self, ctx: Context, *, query: str):
		...
		try:
			...
			commands = ["cd ~/coffin-rewrite/proxy-server", "go run ./add6.go --ipv6 2602:fa48:0:2:: --v6_n 300 --interface eno49", "sudo docker container restart ipv6-proxy"]
		...
		embeds = [
			...
			.set_footer(text=f"Page {i}/{len(results.results)} of Google Images", icon_url="https://cdn4.iconfinder.com/data/icons/logos-brands-7/512/google_logo-google_icongoogle-512.png",)
	...
	@command(name = "google", description = "Search the largest search engine on the internet", example = ",google purple")
	async def google(self, ctx: Context, *, query: str):
		...
		try:
			...
			commands = ["cd ~/coffin-rewrite/proxy-server", "./add6 --ipv6 2602:fa48:0:2:: --v6_n 300 --interface eno49", "sudo docker container restart ipv6-proxy"]
			for command in commands:
				await create_subprocess_shell(command)
			try:
				results = await self.bot.services.brave.search(query, safe)
		...
		if results.main_result:
			if results.main_result.title:
				try:
					embed = Embed(color = self.bot.color, title = results.main_result.title, url = results.main_result.url or self.bot.config['domain'], description = results.main_result.description).set_footer(text=f"Page 1/{pages+1} of Google Search {'(Safe Mode)' if safe else ''}",
	...
	@command(name = "userinfo", aliases = ["ui", "user", "whois"], description = "View information about a member or yourself", example = ",userinfo @kuzay")
	async def userinfo(self, ctx: Context, member: Optional[Union[Member, User]] = Author):
		if not member:
		...
		if isinstance(member, Member):
			position = sorted(ctx.guild.members, key=lambda m: m.joined_at).index(member) + 1
			guild_roles = [r for r in ctx.guild.roles][::-1]
			roles = [r for r in member.roles if not r.is_default() and not r.is_integration()]
			roles = sorted(roles, key = lambda x: guild_roles.index(x), reverse = False)
			roles_to_mention = roles[:5]
			if len(roles) != 0:
				embed.add_field(name = f"Roles ({len(roles)})", value = ", ".join(m.mention for m in roles_to_mention) + ("..." if len(roles) > 5 else ""))
	...
	@group(name = "timezone", aliases = ["tz", "time", "usertime"], description = "get the timezone of other members", example = ",timezone @jonathan", invoke_without_command = True)
	async def timezone(self, ctx: Context, *, user: Union[User, Member] = None):
		user = user or ctx.author
		if not (user_timezone := await self.bot.db.fetchval("""SELECT timezone FROM user_config WHERE user_id = $1""", user.id)):
			raise CommandError(f"no timezone found for {'you' if user.id is ctx.author.id else user.mention}")
		now = datetime.now(tz = timezone(user_timezone))
		return await ctx.normal(f"its currently {utils.format_dt(now, style='F')} for **{str(user)}**", emoji = "ðŸ•")
	
	@timezone.command(name = "set", aliases = ["settz"], description = "set your timezone based on location", example = ",timezone set Los Angeles CA")
	async def timezone_set(self, ctx: Context, *, location: Timezone):
		await self.bot.db.execute("""INSERT INTO user_config (user_id, timezone) VALUES($1, $2) ON CONFLICT(user_id) DO UPDATE SET timezone = excluded.timezone""", ctx.author.id, str(location))
		return await ctx.success(f"your timezone has been set as **{str(location)}**")
	
	@timezone.command(name = "reset", aliases = ["clear", "unset", "cl"], description = "remove your timezone")
	async def timezone_reset(self, ctx: Context):
		try:
			await self.bot.db.execute("""UPDATE user_config SET timezone = NULL WHERE user_id = $1""", ctx.author.id)
		except Exception:
			raise CommandError("you haven't set your **timezone**")
		return await ctx.success("successfully reset your **timezone**")
	
	@timezone.command(name = "list", aliases = ["ls", "view", "show"], description = "view the times of members in the server")
	async def timezone_list(self, ctx: Context):
		timezones = await self.bot.db.fetch("""SELECT user_id, timezone FROM user_config WHERE user_id = any($1::bigint[])""", [c.id for c in ctx.guild.members])
		...
		rows = [f"`{i}` {get_row(record)}" for i, record in enumerate(timezones, start = 1)]
		embed = Embed(title = "Timezones").set_author(name = str(ctx.author), icon_url = ctx.author.display_avatar.url)
	...
	@command(name = "ask", aliases = ["chatgpt", "gpt"], description = "ask AI a question or to solve a prompt", example = ",ask what is purple")
	async def ask(self, ctx: Context, *, prompt: str):
		...
		message = message.replace("BLACKBOX", "RIVAL")
		message = message.replace("BLACKBOX.AI", "RIVAL.ROCKS")
