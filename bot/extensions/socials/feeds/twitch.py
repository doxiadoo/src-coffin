from DataProcessing.models.Twitch import Stream, Channel
...
class Twitch(Feed):
    ...
    async def dispatch(self, stream: Stream, records: List[Record]):
        ...
        embed = Embed(title = shorten(stream.title, 256), color = Color.from_str("#6441a5"), url = f"https://twitch.tv/{stream.user_login}")
        embed.set_author(name = stream.user_login, url = f"https://twitch.tv/{stream.user_login}", icon_url = channel.channel.profile_image_url)
    ...
    async def start(self) -> None:
        ...
        while True:
            ...
            if self.scheduled_deletion:
                await self.bot.db.execute("""DELETE FROM feeds.twitch WHERE channel_id = ANY($1::BIGINT[])""", self.scheduled_deletion)
            await sleep(300)
