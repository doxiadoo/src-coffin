...
async def to_message(self: TikTokPostResponse, ctx: Context):
    embed = Embed(description = unidecode_rs(self.itemInfo.itemStruct.contents[0].desc if self.itemInfo.itemStruct.contents else ""), color = Color.from_str("#00001"))
    embed.set_author(name = self.itemInfo.itemStruct.author.uniqueId, icon_url = self.itemInfo.itemStruct.author.avatarLarger)
    ...
    if self.itemInfo.itemStruct.imagePost:
        ...
        for i, image in enumerate(self.itemInfo.itemStruct.imagePost.images, start = 1):
            e = embed.copy()
            e.set_footer(text = f"{footer_text} ∙ Page {i}/{total}", icon_url = "https://seeklogo.com/images/T/tiktok-icon-logo-1CB398A1BD-seeklogo.com.png")
        ...
        if self.itemInfo.itemStruct.video:
            ...
            async with ClientSession() as session:
                async with session.get(self.itemInfo.itemStruct.video.playAddr, **await ctx.bot.services.tiktok.tt.get_tiktok_headers()) as response:
