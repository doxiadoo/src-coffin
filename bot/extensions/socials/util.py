...
from typing import Literal, Any, Optional, List, Union, Generator, AsyncGenerator
...
def to_embed(self: Item, ctx: Context) -> Union[Embed, List[Embed]]:
    ...
    embed.set_author(name = f"{str(self.owner.full_name)} (@{str(self.owner.username)})", icon_url = self.owner.profile_pic_url or "https://eros.rest/static/Default_pfp.jpg")
    ...
    if self.media_type == 8:
        ...
        for i, media in enumerate(self.carousel_media, start = 1):
            ...
            e.set_footer(text = f"{footer_text} ∙ Page {i}/{len(self.carousel_media)}", icon_url = icon_url)
            embeds.append(e)
        return embeds
