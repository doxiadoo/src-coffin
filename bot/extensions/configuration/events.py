...
from data.variables import MESSAGE_EVENT_ALLOWED_MENTIONS, COLORHEX, HEX_COOLDOWN, TRANSCRIBE_COOLDOWN, TRACKER_COOLDOWN
...
from asyncio import sleep, gather, ensure_future, Lock
...
def to_data(guild: Guild):
    ...
    guild_payload = {
        ...
        'channels': [channel.to_data() for channel in guild._channels],  # Assuming _channels is defined
        'voice_states': [state.to_data() for state in guild._voice_states.values()],  # Assuming _voice_states is defined
        'threads': [thread.to_data() for thread in guild._threads],  # Assuming _threads is defined
        'stage_instances': [instance.to_data() for instance in guild._stage_instances.values()],  # Assuming _stage_instances is defined
        'guild_scheduled_events': [event.to_data() for event in guild._scheduled_events.values()],  # Assuming _scheduled_events is defined
        'soundboard_sounds': [sound.to_data() for sound in guild._soundboard_sounds],  # Assuming _soundboard_sounds is defined
    }
...
class ConfigurationEvents(Cog):
    ...
    async def boost_dispatcher(self, before: Member, after: Member):
        if before.guild.premium_subscription_count < after.guild.premium_subscription_count or not before.premium_since:
            if after.premium_since:
                member = after
                if not (config := await self.bot.db.fetchrow("""SELECT boost_channel, boost_message FROM config WHERE guild_id = $1""", member.guild.id)):
                    return
                if not config.boost_channel or not config.boost_message:
                    return
                if not (channel := member.guild.get_channel(config.boost_channel)):
                    return
                self.bot.dispatch("boost", member.guild, member, channel, config.boost_message)
    ...
    async def check_message(self, message: Message, config: Record) -> bool:
        ...
        if (len(message.content.split("\n")) >= config.ladder_threshold) and (config.ladder_status is True):
            self.bot.dispatch("ladder_typing", message, config)
            return True
        
        if (len(message.content) >= config.flood_threshold) and (config.flood_status is True):
            self.bot.dispatch("text_flood", message, config)
            return True
        
        if await self.bot.object_cache.ratelimited(xxh32_hexdigest(f"spam:{message.guild.id}:{message.author.id}"), config.threshold, 10) != 0:
    ...
    async def on_valid_message(self, ctx: Context):
        if not (config := await self.bot.db.fetchrow("""SELECT timeout, threshold, whitelisted, ladder_status, ladder_threshold, flood_status, flood_threshold, status FROM antispam WHERE guild_id = $1""", ctx.guild.id)):
            return
    ...
    async def event_dispatcher(self, ctx: Context):
        ...
        async def voice_check():
            async with self.locks[f"transcribe:{ctx.guild.id}"]:
                voice_attachments = [attachment for attachment in ctx.message.attachments if attachment.is_voice_message()]
                if not voice_attachments:
                    return
                check = (await self.bot.db.fetchval("""SELECT transcription FROM config WHERE guild_id = $1""", ctx.guild.id) or False)
                if not check or check is False:
                    return
                ratelimit = await self.bot.object_cache.ratelimited(f"transcribe:{ctx.message.guild.id}", *TRANSCRIBE_COOLDOWN)
                if ratelimit != 0:
        ...
        async def hex_code_check():
            async with self.locks[f"color_hex:{ctx.channel.id}"]:
                if not (match := re.search(COLORHEX, ctx.message.content)):
                    return
                check = (await self.bot.db.fetchval("""SELECT auto_hex FROM config WHERE guild_id = $1""", ctx.guild.id) or False)
                if not check or check is False:
                    return
                ratelimit = await self.bot.object_cache.ratelimited(f"hex:{ctx.message.channel.id}", *HEX_COOLDOWN)
                if ratelimit != 0:
                    return # or await sleep(ratelimit) # if you just want to sleep it
    ...
    async def punish(self, message: Message, config: Record, event_type: int):
        async with self.locks[f"punishment:{message.author.id}"]:
            ...
            if message.author.top_role.actual_position <= message.guild.me.top_role.actual_position:
                return
            ...
            if await self.bot.object_cache.ratelimited(f"punishment:{message.guild.id}:{message.author.id}", 1, (config.timeout - 10)) != 0:
                return
            reason = ANTI_SPAM_MAP[str(event_type)]["reason"]
            await message.author.timeout(utcnow() + timedelta(seconds = config.timeout), reason = reason)
    ...
    async def on_voice_message(self, message: Message, voice_message: Attachment):
        ...
        await message.reply(embed = Embed(description = transcription).set_author(name = str(message.author), icon_url = message.author.display_avatar.url), allowed_mentions = AllowedMentions(replied_user = False))
    ...
    async def on_hex_submission(self, message: Message, hex_code: str):
        ...
        return await message.reply(embed=embed, allowed_mentions = MESSAGE_EVENT_ALLOWED_MENTIONS)
    ...
    async def on_vanity_change(self, vanity: str):
        if not (trackers := await self.bot.db.fetch("""SELECT guild_id, channel_ids FROM trackers WHERE tracker_type = $1""", "vanity")):
            return
    ...
    async def emit_vanity_change(self, guild: Guild, vanity: str, channel_ids: List[int]):
        async def send_message(channel: GuildChannel, vanity: str):
            ratelimit = await self.bot.object_cache.ratelimited(f"rl:tracker_message:vanity:{guild.id}", *TRACKER_COOLDOWN)
            if ratelimit != 0:
                await sleep(ratelimit)
            return await channel.send(f"Vanity URL available: **/{vanity}**")
        channels = [channel for channel_id in channel_ids if (channel := guild.get_channel(channel_id))]
    ...
    async def on_username_tracker(self, username: str):
        if not (trackers := await self.bot.db.fetch("""SELECT guild_id, channel_ids FROM trackers WHERE tracker_type = $1""", "username")):
            return
        for tracker in trackers:
            if not (guild := self.bot.get_guild(tracker.guild_id)):
                continue
            ensure_future(self.emit_username_change(guild, username, tracker.channel_ids))

    async def emit_username_change(self, guild: Guild, username: str, channel_ids: List[int]):
        async def send_message(channel: GuildChannel, username: str):
            ratelimit = await self.bot.object_cache.ratelimited(f"rl:tracker_message:username:{guild.id}", *TRACKER_COOLDOWN)
            if ratelimit != 0:
                await sleep(ratelimit)
    ...
    async def not_whitelisted(self, member: Member):
        ...
        async with self.locks[member.guild.id]:
            if (dm := await self.bot.db.fetchval("""SELECT whitelist_dm FROM config WHERE guild_id = $1""", member.guild.id)):
                await self.bot.send(member, dm, True)
            if await self.bot.object_cache.ratelimited(xxh32_hexdigest(f"unwhitelisted:{member.id}-{member.guild.id}"), 3, 100000) != 0:
                return await member.ban(reason = "Member tried to join multiple times while being unwhitelisted")
    ...
    async def on_raid(self, member: Member, reason: str):
        async with self.locks[f"raid:{member.guild.id}"]:
            rl = await self.bot.object_cache.ratelimited(f"raid_kick:{member.guild.id}", 4, 5)
            if rl != 0:
                await sleep(rl)
    ...
    async def on_auto_role(self, member: Member):
        ...
        if not (role_ids := await self.bot.db.fetchval("""SELECT auto_roles FROM config WHERE guild_id = $1""", member.guild.id)):
            return
        ...
        for role in role_ids:
            if not (r := get_role(role)):
                continue
            if r in roles:
                continue
            roles.append(r)
        return await member.edit(roles = set(roles), reason = "Auto Role")
    
    @Cog.listener("on_welcome")
    async def welcome_message(self, guild: Guild, member: Member, channel: TextChannel, code: str):
    ...
    async def boost_message(self, guild: Guild, member: Member, channel: TextChannel, code: str):
        rl = await self.bot.object_cache.ratelimited(f"message_event:{guild.id}", 3, 10)
        if rl != 0:
            await sleep(rl)
        return await self.bot.send_embed(channel, code, user = member)
    ...
    async def leave_message(self, guild: Guild, member: Member, channel: TextChannel, code: str):
        rl = await self.bot.object_cache.ratelimited(f"message_event:{guild.id}", 3, 10)
        if rl != 0:
            await sleep(rl)
        return await self.bot.send_embed(channel, code, user = member)
