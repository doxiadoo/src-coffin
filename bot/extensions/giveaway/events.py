...
class GiveawayEvents(Cog):
    ...
    async def giveaway_ended(self, guild: Guild, channel: TextChannel, giveaway: Record):
        ...
        if len(giveaway.entries) == 0:
            await (message.reply("No entries for this giveaway.") if message else channel.send("No entries for this giveaway."))
        ...
        win_message = await (message.reply(embed = embed) if message else channel.send(embed = embed))
