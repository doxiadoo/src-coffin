...
from .client import BaseModel, ClientSession, CS
import hashlib
...
def sign(method, token):
	string = "api_key" + CONFIG["Authorization"]["lastfm"]["login"] + "method" + method + "token" + token + secret
...
async def request(
    ...
    **kwargs,
    ...
    payload.update(
        {
            "api_key": payload.get("api_key") or CONFIG["Authorization"]["lastfm"]["default"],
            "format": "json",
        }
    )
    data = None
...
async def index(
    ...
    if isinstance(user, Record):
        user = await request(session = session, payload = {"method":"user.getinfo","username": user.username if not isinstance(user, str) else user}, slug="user",)
...
#        pages = ceil(int(getattr(user, f"{library[:-1]}_count", 0)) / 1000)
 #       if pages < 1: page = 1
        #logger.info(pages)
        #logger.info(library)
        items = await api_request(params = {"method": f"user.gettop{library}", "limit": 1000, "period": "overall", "username": user.username if not isinstance(user, str) else user})
        ...
        for i, page in enumerate(range(2, pages+1), start = 0):
            ...
            attempts = 0
            while True:
                p = await api_request(params = {"method": f"user.gettop{library}","username": user.username if not isinstance(user, str) else user,"limit": 1000,"page": i + 1, "period": "overall"})
...
async def login(session: ClientSession, ctx: Context):
    ...
    token = await request(session, payload = {"method": "auth.getToken", "api_key": key}, slug="token", raise_for={404: f"[**{ctx.author.display_name}**]({URL(f'https://last.fm/user/{user_id}')}) is not a valid **Last.fm** account"},)
    link = f'https://www.last.fm/api/auth?api_key={key}&token={token}&cb=https://api.coffin.bot/lastfm?user_id={user_id}'
    try:
        message = await ctx.author.send(embed = Embed(color = 0xd31f27, description = f"""Authorize **coffin** to use your Last.fm account [here]({link}). Your library's data will be indexed to power the whoknows commands and other commands.\n\nIf you want to remove your account with **coffin**, run `lastfm logout` and visit your settings on [Last.fm](https://last.fm/settings/applications/) to unauthorize our application"""))
    except:
    ...
    for attempt in range(3):
        ...
        try:
            if data := await request(session, payload = {"method": "auth.getSession", "token": token, "api_key": key, "api_sig": signature, "format": "json"}, slug = "session"):
                logger.info(data)
                await ctx.bot.db.execute("""INSERT INTO lastfm_data (user_id, username, key, token) VALUES($1, $2, $3, $4) ON CONFLICT(user_id) DO UPDATE SET username = excluded.username, key = excluded.key, token = excluded.token""", ctx.author.id, data.name, data.key, token)    
                ...
                await message.edit(embed = Embed(color = 0xd31f27, description = f"your lastfm username has been set as **{data.name}**"))
                return True
        except Exception as e:
...
async def scrobble(session: ClientSession, ctx: Context, query: str):
     if not (data := await ctx.bot.db.fetchrow("""SELECT * FROM lastfm_data WHERE user_id = $1""", ctx.author.id)):
          return
