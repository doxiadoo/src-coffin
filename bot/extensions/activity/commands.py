...
from discord import Member, User, Guild, Client, Embed, ClientUser, File, ActivityType
...
class Activity(Cog):
	...
	def get_member(self, user_id: int):
		...
		return next((m for m in user.mutual_guilds if m.get_member(user.id)), None) if user else None

	async def write_usage(self, before: Union[Member, User], after: Union[Member, User]):
		async with self.locks[before.id]:
			with suppress(KeyError):
				...
				status_durations = { "online": 0, "offline": 0, "idle": 0, "dnd": 0, "streaming": 0}
				...
				""", before.id, *status_durations.values())
	...
	async def screentime(self, ctx: Context, *, member: Optional[Member] = None):
		...
		data = await self.bot.db.fetchrow(
			"SELECT online, idle, dnd, offline, streaming FROM screentime WHERE user_id = $1", member.id
		)
		...
		async with self.locks[member.id]:
			...
			if str(member.id) in self.data:
				...
				if status in ["online", "idle", "dnd"]:
					dataset[["online", "idle", "dnd"].index(status)] += now - self.data[str(member.id)]
			...
			dataset = [max(d, 60) for d in dataset]  # Ensure no duration is less than 60 seconds
			online = dataset[0]
			idle = dataset[1]
			dnd = dataset[2]
			offline = dataset[3]
			...
			await ctx.send(file = file, embed = Embed(title = f"""{'your' if member == ctx.author else f"{str(member)}{end}"} activity""", url = member.url).set_image(url = "attachment://chart.png").set_author(name = str(ctx.author), icon_url = ctx.author.display_avatar.url))
		return

	@screentime.command(name="clear", aliases=["cl"])
	async def screentime_clear(self, ctx: Context):
		await self.bot.db.execute("DELETE FROM screentime WHERE user_id = $1", ctx.author.id)
