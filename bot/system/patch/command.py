...
def has_permissions(**permissions: Any):
    async def predicate(ctx: Context):
        ...
        if fake:
            ...
            if len(roles) > 0:
                data = await ctx.bot.db.fetch("""SELECT permissions FROM fake_permissions WHERE guild_id = $1 AND role_id = ANY($2)""", ctx.guild.id, roles)
                for fake_perm in data:
                    fake_permissions.extend([d.lower() for d in fake_perm])
        missing_permissions = []
        for permission in permissions:
            if getattr(ctx.author.guild_permissions, permission) is not True and permission.lower() not in fake_permissions:
