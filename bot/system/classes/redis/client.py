...
class CoffinRedis(Redis):
    def __init__(self, *a, **ka):
        self._locks_created: Dict[Union[str, bytes, memoryview], CoffinLock] = {}
    ...
    async def random(self, table: str, decode: Optional[bool] = True) -> Any:
        value = await self.eval(RANDOM_CHOICE_SCRIPT, 1, table)
        if decode:
