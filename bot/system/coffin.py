...
# make_dask_sink("coffin") # logger overwritting
import discord_ios # type: ignore
import tuuid
import traceback
import os

from contextlib import suppress
from discord.ext.commands import AutoShardedBot, when_mentioned_or, BotMissingPermissions
from discord import Intents, AllowedMentions, Message, Embed, HTTPException, abc, utils, Member, Guild, User, AuditLogEntry, Activity, ActivityType, Streaming
...
from .classes.redis import CoffinRedis
...
from discord.globals import get_global, set_global
...
class Coffin(AutoShardedBot):
    def __init__(self, *args, **kwargs):
        super().__init__(
            ...
            #activity=Activity(name=" ", state=" ", url='https://twitch.tv/coffin', type=ActivityType.custom),
        ...
        self.runner = RebootRunner(self, ["extensions"])
        self.loaded = False
    ...
    async def get_prefix(self: "Coffin", message: Message):
        server = await self.db.fetchval("""SELECT prefix FROM config WHERE guild_id = $1""", message.guild.id)
        user = await self.db.fetchval("""SELECT prefix FROM user_config WHERE user_id = $1""", message.author.id)
        if not server:
            server = ","
        if user:
    ...
    async def setup_dask(self: "Coffin"):
        self.dask = await start_dask(self, "127.0.0.1:8787")


    def get_command_dict(self: "Coffin") -> list:
    ...
    async def setup_database(self: "Coffin") -> bool:
        ...
        if len(failed) == 0:
            logger.info(f"Executed All of the {len(queries)} Database Queries Successfully")
        else:
            logger.info(f"Failed to do queries to the following tables {', '.join(f[0] for f in failed)}")
            errors = "\n".join(f"{f[0]} - {f[1]}" for f in failed)
            logger.info(f"Errors: \n{errors}\n{query}")
        return True
    
    async def __load(self: "Coffin", cog: str):
        try:
    ...
    async def load_cogs(self: "Coffin"):
        tasks = []
        inner_files = ["commands", "events"]
        for path in Path("extensions/").glob("*"):
            if "__pycache__" not in str(path):
                cog_path = "extensions." + str(path).split("extensions/")[1].replace("/", ".").replace("\\", ".")
    ...
    async def setup_hook(self: "Coffin"):
        ...
        self.redis.bot = self
        await self.redis.setup_pubsub(channel = "coffin1")
        await setup(self)
        await self.setup_database()
        self.check(self.command_check)


    async def command_check(self: "Coffin", ctx: Context):
        COOLDOWN_MAPPING = {
            "guild": ctx.guild.id,
        ...
        missing_perms = []
        if await self.db.fetchrow("""SELECT * FROM blacklists WHERE object_id = $1 AND object_type = $2""", ctx.author.id, "user"):
            return False
        if await self.db.fetchrow("""SELECT * FROM blacklists WHERE object_id = $1 AND object_type = $2""", ctx.guild.id, "guild"):
            return False
        ...
        if cooldown_override := ctx.command.extras.get("cooldown"):
            ...
            if retry_after := await ctx.bot.object_cache.ratelimited(
                f"rl:{ctx.command.qualified_name}{c}:{COOLDOWN_MAPPING.get(cooldown_type.lower())}", limit, timeframe
            ):
                raise commands.CommandOnCooldown(ctx.command, retry_after, cooldown_type)
        if ctx.command.qualified_name in (await self.db.fetchval("""SELECT disabled_commands FROM config WHERE guild_id = $1""", ctx.guild.id) or []):
            return False
        return True

    async def create_embed(self: "Coffin", code: str, **kwargs: Any):
    ...
    async def user_count(self: "Coffin") -> int:
        return sum(i for i in self.get_all_members())
    
    async def guild_count(self: "Coffin") -> int:
        return len(self.guilds)
    
    async def channel_count(self: "Coffin") -> int:
        return sum(len(i.channels) for i in self.guilds)
    
    async def role_count(self: "Coffin") -> int:
        return sum(len(i.roles) for i in self.guilds)
    
    

    async def send_embed(self: "Coffin", destination: Union[Context, abc.GuildChannel], code: str, **kwargs: Any):
        ...
        try:
            ...
            return await destination.send(embed = Embed(color = CONFIG["colors"]["warning"], description = str(error)))
            raise
    

    async def send_exception(self: "Coffin", ctx: Context, exception: Exception):
        code = tuuid.tuuid()
        ...
        return await ctx.send(
            content=f"{code}",
            embed=Embed(
                description=f"{CONFIG['emojis']['warning']} {ctx.author.mention}: Error occurred while performing command **{ctx.command.qualified_name if ctx.command else 'repost'}**. Use the given error code to report it to the developers in the [support server](https://{CONFIG.get('domain')}/discord)",
                color=0xe69705,
            ),
        )

    @cache(key = "emojis", ttl = 300)
    async def get_emojis(self: "Coffin"):
        return await self.fetch_application_emojis()
    
    @lock("DM")
    async def send(self: "Coffin", member: Member, content: str, embed_code: Optional[bool] = False, *args: Any, **kwargs: Any):
        ...
        if embed_code:
            try:
                return await self.send_embed(member, content, embed_code, *args, **kwargs)
            except Exception:
    ...
    async def check_member(self, member: Member, guild: Guild, config: Record, dispatch: Optional[bool] = True) -> tuple:
        ...
        if config.raid_status is True and member.id not in whitelist:
            if datetime.now() > config.raid_expires_at:
                await self.db.execute("""UPDATE antiraid SET raid_triggered_at = NULL, raid_expires_at = NULL WHERE guild_id = $1""", guild.id)
            else:
        ...
        if await self.object_cache.ratelimited(f"raid:{guild.id}", config.join_threshold, 60) != 0:
            expiration = datetime.now() + timedelta(minutes = 10)
            await self.db.execute("""INSERT INTO antiraid (guild_id, raid_status, raid_triggered_at, raid_expires_at) VALUES($1, $2, $3, $4) ON CONFLICT(guild_id) DO UPDATE SET raid_status = excluded.raid_status, raid_triggered_at = excluded.raid_triggered_at, raid_expires_at = excluded.raid_expires_at""", guild.id, True, datetime.now(), expiration)
    ...
    async def check_raid(self, member: Member) -> tuple:
        guild = member.guild
        async def execute():
            if not (config := await self.db.fetchrow("""SELECT * FROM antiraid WHERE guild_id = $1""", guild.id)):
                return False
            value = False
        ...
        _ = await execute()
        if not _:
            whitelist = await self.db.fetchrow("""SELECT whitelist, whitelist_whitelist FROM config WHERE guild_id = $1""", member.guild.id)
            if whitelist:
    ...
    async def create_emoji(self: "Coffin", name: str, data: bytes):
    ...
    async def on_message(self: "Coffin", message: Message):
        with suppress(AttributeError):
            if not (message.author.bot) and (message.channel.permissions_for(message.guild.me).send_messages) and (message.guild) and self.is_ready():
                ...
                if not ctx.valid:
                    ...
                    if await self.db.fetchval("""SELECT reposting FROM config WHERE guild_id = $1 AND reposting = TRUE""", ctx.guild.id):
                        repost(self, message)
                self.dispatch("afk_check", ctx)
                if message.mentions_bot(strict=True):
                    if await self.object_cache.ratelimited(f"prefix_pull_message:{ctx.guild.id}", 3, 5) == 0:
                        guild_prefix, user_prefix = await ctx.display_prefix(False)
                        user_prefix_value = f"\nYour Custom Prefix is `{user_prefix}`" if user_prefix else ""
                        return await ctx.normal(f"Guild Prefix is `{guild_prefix}`{user_prefix_value}")
    ...
    async def on_command_error(self: "Coffin", ctx: Context, exception: Exception):
        return await self.errors.handle_exceptions(ctx, exception)

    async def dispatch_welcome(self: "Coffin", member: Member) -> None:
        if not (config := await self.db.fetchrow("""SELECT welcome_message, welcome_channel FROM config WHERE guild_id = $1""", member.guild.id)):
            return
        ...
        if not (welcome_channel := member.guild.get_channel(config.welcome_channel)):
            return
        self.dispatch("welcome", member.guild, member, welcome_channel, config.welcome_message)

    async def on_member_join(self: "Coffin", member: Member) -> None:
    ...
    async def on_member_remove(self: "Coffin", member: Member) -> None:
        if member.premium_since:
            self.dispatch(
                "member_unboost",
                member,
            )
        if not (config := await self.db.fetchrow("""SELECT leave_message, leave_channel FROM config WHERE guild_id = $1""", member.guild.id)):
            return
        ...
        if not (leave_channel := member.guild.get_channel(config.leave_channel)):
            return
        self.dispatch("leave", member.guild, member, leave_channel, config.leave_message)

    async def on_user_update(self: "Coffin", before: User, after: User):
        if not all([
    ...
    async def on_member_update(
        self, before: Member, member: Member
    ) -> None:
        ...
        if booster_role := member.guild.premium_subscriber_role:
            if (booster_role in before.roles) and (booster_role not in member.roles):
                ...
                if not (config := await self.db.fetchrow("""SELECT boost_message, boost_channel FROM config WHERE guild_id = $1""", member.guild.id)):
                    return
                if not config.boost_message or not config.boost_channel:
                    return
                if not (boost_channel := member.guild.get_channel(config.boost_channel)):
                    return
                self.dispatch("boost", member.guild, member, boost_channel, config.boost_message)
        else:
    ...
    async def get_context_cached(self: "Coffin", message: Message) -> Context:
        return await super().get_context(message, cls = Context)
    
    async def get_context(self: "Coffin", message: Message, **kwargs) -> Context:
    ...
    async def fetch_message(self: "Coffin", channel: abc.GuildChannel, message_id: int):
    ...
    async def get_reference(self: "Coffin", message: Message):
    ...
    async def setup_emojis(self: "Coffin"):
        if not CONFIG["emojis"]["interface"].get("lock", "") not in ('', ""):
            for p in Path("../assets/emojis").glob("*"):
                ...
                if path_name.lower() == "embeds":
                    ...
                    if list(CONFIG["emojis"].get(path_name.lower(), {}).values())[0] != '':
                        continue
