...
def generate_large_text_file(file_name, size_in_gb, num_threads=8):
    ...
    chunks = [(i * chunk_size, min((i + 1) * chunk_size, total_size)) for i in range(num_chunks)]

    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        # Submit write tasks to the thread pool
        futures = [executor.submit(write_chunk, file_name, start, end) for start, end in chunks]
